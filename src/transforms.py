...
class Rescale(object):
    ...
    def __call__(self, img):
        ...
        img1 = cv2.resize(img[1, :, :], (224, 224))
        img2 = cv2.resize(img[2, :, :], (224, 224))
        img_final = np.empty((3, 224, 224), dtype=img.dtype)
        img_final[0, :, :] = img0
        img_final[1, :, :] = img1
        img_final[2, :, :] = img2
...
class CutOutWrapper:
    def __init__(self, batch_size=5, prob=0.5):
        self.cutout = CutOut(batch_size=batch_size)
        self.prob = prob
